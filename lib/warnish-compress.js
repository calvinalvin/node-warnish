...
module.exports = function compress(options) {
    ...
    if (options.client) {
        ...
        redisClient
        .on('ready', function () {
            console.log('warnish-compress'.blue + ': redis client is ready %s:%s', this.host, this.port);
        })
        .on('connect', function () {
            console.log('warnish-compress'.blue + ': redis client is connected %s:%s', this.host, this.port);
        })
        ...
        res.on('header', function () {
            ...
            stream.on('data', function (chunk) {
                ...
                if (ignoreVerbs.indexOf(req.method) == -1) {
                    ...
                    console.log('warnish-compress'.blue + ': saving compressed response into redis. KEY: %s', cacheKey);
