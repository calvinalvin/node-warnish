...
module.exports = function accelerate(options) {
    ...
    if (options.client) {
        ...
        redisClient
        .on('ready', function () {
            console.log('warnish-accelerate:'.cyan + ' redis client is ready %s:%s', this.host, this.port);
        })
        .on('connect', function () {
            console.log('warnish-accelerate:'.cyan + ' redis client is connected %s:%s', this.host, this.port);
        })
        .on('error', function (err) {
            throw new Error('warnish-accelerate: Error '.red + err);
        });
    }
        ...
        redisClient.exists(cacheKey, function (err, reply) {
            ...
            if (reply === '1') {
                redisClient.hgetall(headerKey, function (err, reply) {
                    ...
                    console.log('cache HIT '.cyan + cacheKey);
