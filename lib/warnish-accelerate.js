...
module.exports = function accelerate(options) {
    ...
    if (options.client) {
        ...
        redisClient
        .on('ready', function () {
            console.log('warnish-accelerate:'.blue + ' redis client is ready %s:%s', this.host, this.port);
        })
        .on('connect', function () {
            console.log('warnish-accelerate:'.blue + ' redis client is connected %s:%s', this.host, this.port);
        })
        ...
        redisClient.exists(cacheKey, function (err, reply) {
            ...
            if (reply === '1') {
                redisClient.hgetall(headerKey, function (err, reply) {
                    ...
                    console.log('cache HIT '.blue + cacheKey);
