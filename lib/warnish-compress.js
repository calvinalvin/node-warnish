/*!
* ~~ warnish ~~
* Connect - compression + cache middleware
* by Calvin Oh, Copyright(c) 2013 Calvin Oh
* MIT Licensed
*
...
var util = require('util')
  , zlib = require('zlib')
  , redis = require('redis')
  , redisWStream = require('./redis-wstream');
...
* Use Warnish Get to retrieve the cached values
*
* Filter:
*
...
* };
*
* warnish defaults to no filtering
*
*
* Options:
*
...
module.exports = function compress(options) {
    ...
    redisClient
    .on('ready', function () {
        console.log('warnish-compress: redis client is ready %s:%s', this.host, this.port);
    })
    .on('connect', function () {
        console.log('warnish-compress: redis client is connected %s:%s', this.host, this.port);
    })
    .on('error', function (err) {
        throw new Error('warnish-compress: ' + err);
    });
    ...
    return function compress(req, res, next) {
        ...
        res.end = function (chunk, encoding) {
            ...
            if (stream && verbs.indexOf(req.method) == -1) {
                ...
                console.log('warnish-compress: saving compressed response into redis');
                stream.pipe(redisWStream(redisClient, cacheKey, cacheExpires));
            ...
            // don't store raw responses. will unnecessarily bloat redis with uncompressed data. 
            // most modern browsers support some form of compression
